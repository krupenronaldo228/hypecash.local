...
class Packages extends ADMIN_Controller {
	...
	public function __construct ()
	{
		...
		$this->model = $this->packages_model;
	...
	public function create()
	{
		...
		if($_POST)
		{
			try
			{
				...
				set_flash('result', action_result('success', fa5s('check mr5').' Пакет <strong>"'.$insert['title'].'"</strong> успешно добавлен!', true));
	...
	public function edit()
	{
		...
		if(empty($item))
		{
			set_flash('result', action_result('error', fa5s('exclamation-triangle mr5').' Пакет не найден!', true));
			redirect('admin/'.$this->page);
		}
		...
		if($_POST)
		{
			try
			{
				...
				set_flash('result', action_result('success', fa5s('check mr5').' Пакет <strong>"'.$insert['title'].'"</strong> успешно обновлен!', true));
	...
	public function delete()
	{
		...
		try
		{
			if($_POST && $this->input->post('delete') == 'delete')
			{
				$item = $this->model->getItem('id', $id);
				if(empty($item))
					throw new Exception(fa5s('exclamation-triangle mr5').' Пакет не найден!');
		...
		if($error)
		{
			...
			set_flash('result', action_result('success', fa5r('trash-alt mr5').' Пакет <strong>"'.$item['title'].'"</strong> успешно удален!', true));
		}
	...
	public function ajaxImageDelete()
	{
		...
		try
		{
			if($this->input->post('delete_img') == 'delete' && !empty($type))
			{
				$item = $this->model->getItem('id', $id);
				if(empty($item))
					throw new Exception('Пакет не найдена!');
